...
@bp_users.route('/me/following', methods=['GET'])
@authenticate(token_auth)
...
@bp_users.route('/me/followers', methods=['GET'])
@authenticate(token_auth)
@paginated_response(users_schema, order_by=User.username)
def my_followers():
...
@bp_users.route('/me/following/<int:id>', methods=['GET'])
@authenticate(token_auth)
...
@bp_users.route('/me/following/<int:id>', methods=['POST'])
@authenticate(token_auth)
@response(EmptySchema, status_code=204,
...
@bp_users.route('/me/following/<int:id>', methods=['DELETE'])
@authenticate(token_auth)
@response(EmptySchema, status_code=204,
...
@bp_users.route('/users/<int:id>/following', methods=['GET'])
@authenticate(token_auth)
@paginated_response(users_schema, order_by=User.username)
@other_responses({404: 'User not found'})
def following(id):
...
@bp_users.route('/users/<int:id>/followers', methods=['GET'])
@authenticate(token_auth)
@paginated_response(users_schema, order_by=User.username)
@other_responses({404: 'User not found'})
def followers(id):
